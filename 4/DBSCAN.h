...
class DBSCAN {
    ...
    std::vector<int> getNeighbors( std::vector<T>& Ts, int index );
...
template<typename T>
double DBSCAN<T>::distance( T& p1, T& p2 ){
    auto d = p1-p2;
    return std::sqrt(d*d);
}
...
template<typename T>
std::vector<int> DBSCAN<T>::getNeighbors( std::vector<T>& Ts, int index ){
    std::vector<int> neighbors;
    for( int i=0; i<Ts.size(); ++i ){
        if( distance( Ts[index], Ts[i]) < m_epsilon ){
