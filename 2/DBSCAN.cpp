    ...
    for( int i: neighbors ){
        if( labels[i] == 0 ){
            ...
            if( newNeighbors.size() >= minPts ){
                ...
                expandCluster( XYs, labels, i, newNeighbors, clusterCount );
            }
            continue;
        }
...
void DBSCAN(){  // HEADER
    ...
    for( int i=0; i<XYs.size(); i++ ){
        ...
        ++clusterCount;
        expandCluster( XYs, labels, i, neighbors, clusterCount );
    }
}
...
void DBSCAN_render(){ // HEADER
    ...
    // https://en.m.wikipedia.org/wiki/File:Mac_16colors_palette.svg
    ...
    for( int i=0; i<XYs.size(); i++ ){
        auto xy = XYs[i];
        auto label = labels[i];
        auto rgb = label < 0 ? mac_palette[14] : mac_palette[label%16];
